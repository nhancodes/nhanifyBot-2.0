...
import { isValidURL, parseURL, getVideoById } from '../videoAPI/youtube/dataAPI.js';
...
describe('YouTube API Functions', () => {
  ...
  describe('isValidURL', () => {
    it('should validate standard YouTube URLs', () => {
      ...
      expect(isValidURL('https://music.youtube.com/watch?v=dQw4w9WgXcQ')).toBe(true);
    });

    it('should reject invalid YouTube URLs', () => {
      expect(isValidURL('https://www.youtube.com')).toBe(false);
      expect(isValidURL('https://www.youtube.com/playlist?list=123')).toBe(false);
    ...
    it('should handle URLs with extra parameters', () => {
      expect(isValidURL('https://www.youtube.com/watch?v=dQw4w9WgXcQ&list=123&index=5')).toBe(true);
  ...
  describe('parseURL', () => {
    it('should extract video ID from standard YouTube URLs', () => {
      ...
      expect(parseURL('https://m.youtube.com/watch?v=dQw4w9WgXcQ')).toBe('dQw4w9WgXcQ');
    });

    it('should handle URLs with extra parameters', () => {
      expect(parseURL('https://www.youtube.com/watch?v=dQw4w9WgXcQ&list=123&index=5')).toBe('dQw4w9WgXcQ');
      expect(parseURL('https://youtu.be/dQw4w9WgXcQ?si=abc123')).toBe('dQw4w9WgXcQ');
    });
  });
  ...
  describe('getVideoById', () => {
    ...
    it('should handle videos with restrictions', async () => {
      ...
      const nonEmbeddableResult = await getVideoById('embed123', 'fake-api-key');
      ...
      const regionRestrictedResult = await getVideoById('region123', 'fake-api-key');
