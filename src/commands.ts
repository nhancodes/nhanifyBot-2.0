...
export async function playerSkipSong(
	...
	try {
		if (Queue.getPlayingOn() === null) {
			return client.send(
				`PRIVMSG #${auth.BROADCASTER_NAME} : @${chatter}, all queues are empty.`,
			);
		}

		// Remove current song from active queue
		Queue.getPlayingOn() === "nhanify"
			? nhanifyQueue.remove()
			: chatQueue.remove();
		...
		try {
			client.send(
				`PRIVMSG #${auth.BROADCASTER_NAME} : @${chatter}, there was an error skipping the song.`,
			);
		} catch (e) {
 ...
 */
export async function playerSkipPlaylist(
	webSocketServerClients: Set<WebSocket>,
	client: WebSocket,
	nhanifyQueue: Queue,
	chatter: string,
	...
	try {
		if (nhanify && Queue.getPlayingOn() === "nhanify") {
			...
			client.send(
				`PRIVMSG #${auth.BROADCASTER_NAME} : @${chatter}, No playlist to skip.`,
			);
		}
	} catch (error) {
		...
		try {
			client.send(
				`PRIVMSG #${auth.BROADCASTER_NAME} : @${chatter}, there was an error skipping the playlist.`,
			);
		} catch (e) {
			console.error("Failed to send error message to client:", e);
		}
...
export async function playerReady(
	...
	try {
		...
		if (!chatQueue.isEmpty()) {
			...
			Queue.setPlayingOn("nhanify");
			ws.send(
				JSON.stringify({ action: "play", queue: nhanifyQueue.getQueue() }),
			);
			await rewards.setRewardsIsPause("nhanify");
		} else {
...
function broadcastToClients(clients: Set<WebSocket>, message: any) {
	...
	if (failedClients > 0) {
		console.warn(
			`Failed to send message to ${failedClients} of ${clients.size} clients`,
		);
	}
}
