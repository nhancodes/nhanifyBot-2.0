...
export async function startTwitchIRCWebSocketClient(IRC_WEBSOCKET_URL: string) {
  ...
  client.on("message", async function(event: Buffer) {
    //console.log({event});
    const message = event.toString('utf8').trim();
    if (message.startsWith('PING :tmi.twitch.tv')) {
      ...
      if (parsedMessage?.command?.type === "botCommand") {
        switch(parsedMessage.command.botCommand) {
          case "testIRC": 
          client.send(`PRIVMSG ${parsedMessage.command.channel} : testIRC recieved\r\n`);
        }
      }
    }
  });
